...
The code is organized under the reusable `src/engine` package, the
Necrosis-specific `src/game/necrosis` package, and the Swing UI in `src/ui`.
...
- `load` rebuilds the game from the JSON game data and replays saved commands
...
Commands such as `save`, `load`, `history`, `help`, and `quit` are not stored in replay history. Older snapshot-style save files can still be read as a migration fallback.
...
The loader validates required fields such as `gameConfig.startRoomId`, room IDs,
item definitions, exit targets, and command aliases. Optional arrays such as
`items`, `enemies`, `interactables`, `npcs`, and `encounterPhases` can be omitted
and will load as empty collections.
...
|   |   |-- core/           Session, state, and command history
|   |   |-- loader/         JSON loading and game configuration
|   |   |-- model/          Rooms, items, enemies, NPCs, exits, and dialogue
|   |   |-- parser/         Command parsing and command registration
|   |   |-- save/           Save data and save/load persistence
|   |   `-- systems/        Reusable gameplay systems
|   |-- game/
|   |   `-- necrosis/
|   |       |-- commands/    Necrosis-specific combat and talk commands
|   |       `-- systems/     Necrosis-specific encounter progression
|   `-- ui/
|       `-- GameWindow.java
|-- saves/                  Runtime save folder; save JSON files are ignored
`-- out/                    Compiled classes generated by IntelliJ or javac
```
